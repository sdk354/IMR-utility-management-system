...
function AddEditBill() {
	...
	// Initial state for a fresh bill
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				// 1. Always load customers for the dropdown
				const response = await customerService.getAllCustomers();
				const allUsers = response.content || response || [];
				const onlyActualCustomers = allUsers.filter(u =>
					u.role?.roleName?.trim().toLowerCase() === 'customer'
				);
				setCustomers(onlyActualCustomers);

				// 2. If editing, fetch bill details and autofill
				if (editId) {
					...
					// 3. If NOT editing (New Bill), reset to initial state
		...
		loadData();
	}, [editId]); // Re-run whenever the URL parameter changes
				...
				<div className="admin-card" style={{ maxWidth: '900px', margin: '0 auto' }}>
					<form onSubmit={handleSubmit}>
						...
						<div className="admin-form-footer" style={{ marginTop: '2.5rem' }}>
							<button type="button" onClick={() => navigate('/admin/billing')} className="admin-btn-secondary">Cancel</button>
