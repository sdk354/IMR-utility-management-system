...
function RegisterMeter() {
	...
	useEffect(() => {
		const loadInitialData = async () => {
			try {
				...
				const onlyActualCustomers = customerData.filter(u =>
					u.role?.roleName?.trim().toLowerCase() === 'customer'
				);
	...
	const handleChange = (e) => {
		...
		setFormData(prev => ({
			...prev,
			[name]: (name === 'utilityTypeId' || name === 'customerId') ? parseInt(value) : value
		}));
	};
	...
	// Configuration for Utility Chips
					...
					<form onSubmit={handleSubmit}>
						<div className="admin-form-grid">
								...
								<div className="status-chip-group">
									{utilityOptions.map((opt) => (
										<button
											...
											// CHANGED: Using opt.color instead of hardcoded blue
							...
							<div className="admin-form-group full-width">
								...
								<div className="status-chip-group">
									{[{l:'Live', v:'Live', c:'#10b981'}, {l:'Under Repair', v:'Under Repair', c:'#f59e0b'}, {l:'Suspended', v:'Suspended', c:'#ef4444'}].map((opt) => (
										<button
											key={opt.v}
											type="button"
						...
						<div className="admin-form-footer" style={{ marginTop: '2.5rem' }}>
							<button type="button" onClick={() => navigate('/admin/meters')} className="admin-btn-secondary">Cancel</button>
