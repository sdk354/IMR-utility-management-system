...
export default function BillDetails() {
	...
	if (!bill) return (<>
		...
		<div className="customer-card">
			...
			<Link to="/customer/bills" className="customer-btn-primary" style={{display: "inline-block"}}>← Back to
				Bills</Link>
		</div>
	</>);
	...
	return (<>
		...
		<div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "2rem"}}>
			...
			<div className="customer-card">
				...
				<div style={{marginBottom: "1.5rem"}}>
					...
					<div style={{
						fontSize: "1.1rem", fontWeight: 600, color: "#5d2e0f"
					}}>{bill.billDate || bill.issuedDate}</div>
				</div>
				<div style={{marginBottom: "1.5rem"}}>
				...
				<div>
					...
					<div
						style={{fontSize: "1.1rem", fontWeight: 600, color: "#f97316"}}>{bill.totalConsumption} kWh
					</div>
				</div>
			</div>
			...
			<div className="customer-card">
				...
				<div style={{marginBottom: "1.5rem"}}>
					...
					<div style={{
						fontSize: "2rem", fontWeight: 900, color: "#f97316"
					}}>Rs. {bill.totalAmount?.toFixed(2)}</div>
				</div>
				<div style={{marginBottom: "1.5rem"}}>
					<div className="small">Status</div>
					<span
						className={`customer-status ${bill.status?.toLowerCase() === "paid" ? "completed" : "pending"}`}
						style={{display: "inline-block", marginTop: "0.5rem"}}>
              {bill.status}
            </span>
				</div>
				{bill.status?.toLowerCase() !== "paid" && (
					<Link to="/customer/payments" className="customer-btn-primary"
		...
		<div className="customer-card">
			<h3 style={{
				color: "#5d2e0f", marginBottom: "1.5rem", borderBottom: "2px solid #f97316", paddingBottom: "0.75rem"
			}}>Bill Breakdown</h3>
