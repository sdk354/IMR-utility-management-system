-- Each procedure should be run separately, without GO, if not using SSMS or sqlcmd.

CREATE OR ALTER PROCEDURE p_InsertUser
    @username VARCHAR(50),
...
CREATE OR ALTER PROCEDURE p_GetUserByID
    @userID INT
    AS
BEGIN
SELECT *
...
CREATE OR ALTER PROCEDURE p_InsertTariff
    @rate DECIMAL(10,4),
    @effectiveFrom DATE,
    @effectiveTo DATE,
...
CREATE OR ALTER PROCEDURE p_UpdateTariff
    @tariffID INT,
    @rate DECIMAL(10,4),
    @effectiveTo DATE
    AS
...
CREATE OR ALTER PROCEDURE p_DeleteTariff
    @tariffID INT
    AS
BEGIN
DELETE FROM Tariff
...
CREATE OR ALTER PROCEDURE p_InsertMeterReading
...
CREATE OR ALTER PROCEDURE p_GetReadingsByMeter
    @meterID INT
    AS
BEGIN
SELECT *
...
CREATE OR ALTER PROCEDURE p_InsertBill
...
CREATE OR ALTER PROCEDURE p_UpdateBillStatus
    @billID INT,
    @status VARCHAR(50)
    AS
BEGIN
...
CREATE OR ALTER PROCEDURE p_InsertBillTariff
    @billID INT,
    @tariffID INT
    AS
BEGIN
...
CREATE OR ALTER PROCEDURE p_UpdateLateFees
    AS
BEGIN
UPDATE Bill
SET lateFee = dbo.CalculateLateFee(billID, GETDATE())
WHERE status <> 'Paid' AND dueDate < GETDATE();
END;
GO

CREATE OR ALTER PROCEDURE AddUtilityType
...
CREATE OR ALTER PROCEDURE AddComplaint
    @userID INT,
    @meterID INT,
...
CREATE OR ALTER PROCEDURE UpdatePaymentMethod
    @paymentID INT,
    @method VARCHAR(50)
    AS
BEGIN
