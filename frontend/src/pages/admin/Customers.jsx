...
function Customers() {
	...
	const [citySearch, setCitySearch] = useState(''); // Changed from cityFilter
	...
	useEffect(() => {
		const fetchCustomers = async () => {
			try {
				...
				const onlyCustomers = data.filter(user =>
					user.role && user.role.roleName && user.role.roleName.toLowerCase() !== 'admin'
				);
	...
	useEffect(() => {
		...
		if (searchTerm) {
			results = results.filter(c =>
				formatToDisplayName(c.username).toLowerCase().includes(searchTerm.toLowerCase()) ||
				c.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
				c.id.toString().includes(searchTerm)
			);
		}

		// NEW: Search both City and Address
		if (citySearch) {
			const term = citySearch.toLowerCase();
			results = results.filter(c =>
				(c.city && c.city.toLowerCase().includes(term)) ||
				(c.street && c.street.toLowerCase().includes(term)) ||
				(c.streetNo && c.streetNo.toLowerCase().includes(term))
			);
		}
	...
	const getStatusClass = (status) => {
		switch (status?.toLowerCase()) {
			case 'active': return 'admin-status completed';
			case 'inactive': return 'admin-status incompleted';
		...
		<div className="admin-page">
			<div className="admin-table-container">
						...
						<div className="filter-grid-container">
							...
							<div className="filter-actions">
								<button
									...
									onClick={() => {setSearchTerm(''); setStatusFilter('All'); setCitySearch('');}}
								>
									Reset All
								</button>
							</div>
				...
				<table className="admin-table">
					...
					<tbody>
					{loading ? (
						<tr><td colSpan="9" style={{textAlign: 'center'}}>Loading customers...</td></tr>
					) : filteredCustomers.length === 0 ? (
						<tr><td colSpan="9" style={{textAlign: 'center'}}>No matches found.</td></tr>
					) : (
