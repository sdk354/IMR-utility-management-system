...
public interface PaymentRepository extends JpaRepository<Payment, Integer> {
    ...
    @Query(value = "SELECT DATENAME(month, paymentDate) as month, SUM(amount) as total " + "FROM Payment " + "WHERE YEAR(paymentDate) = YEAR(GETDATE()) " + "GROUP BY MONTH(paymentDate), DATENAME(month, paymentDate) " + "ORDER BY MONTH(paymentDate)", nativeQuery = true)
