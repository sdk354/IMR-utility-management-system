-- Each function should be run separately, without GO, if not using SSMS or sqlcmd.

CREATE FUNCTION GetFullAddress (@userID INT)
    RETURNS VARCHAR(300)
AS
BEGIN
...
CREATE FUNCTION CalculateCharge (@tariffID INT, @units DECIMAL(18,4))
...
SELECT @rate = rate, @fixedCharge = fixedCharge
FROM Tariff
...
CREATE FUNCTION GetLastReading (@meterID INT)
...
CREATE FUNCTION GetBillStatus (@billID INT)
    RETURNS VARCHAR(50)
AS
BEGIN
...
CREATE FUNCTION CalculateLateFee (@billID INT, @asOfDate DATE)
...
CREATE FUNCTION FindUserByName (@username VARCHAR(50))
    RETURNS TABLE
    AS
RETURN
(
...
CREATE FUNCTION TotalConsumptionByUser (@userID INT)
    RETURNS TABLE
    AS
RETURN
(
    SELECT b.billID, b.totalConsumption, b.totalAmount, b.billDate
    FROM Bill b
    WHERE b.userID = @userID
);
GO

CREATE FUNCTION GetTariffsByBill (@billID INT)
    RETURNS TABLE
    AS
RETURN
(
