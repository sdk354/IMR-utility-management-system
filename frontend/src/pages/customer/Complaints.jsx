...
function CustomerComplaints() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const payload = {
				type: form.issueType, subject: form.subject, description: form.description
			};
			...
			<div className="customer-card">
				...
				<form onSubmit={handleSubmit} style={{maxWidth: "600px"}}>
					<div className="customer-form-group">
						...
						<select id="issue-type" name="issueType" value={form.issueType} onChange={handleChange}
								className="customer-select" required>
					...
					<div className="customer-form-group">
						...
						<textarea id="description" name="description" value={form.description}
								  onChange={handleChange} placeholder="Details..." style={{minHeight: "140px"}}
								  required></textarea>
					</div>
		...
		<div>
			...
			<div className="customer-table-container">
				<table className="customer-table">
					...
					{loading ? (<tr>
					...
					</tr>) : tickets.length > 0 ? (tickets.map(t => (<tr key={t.id}>
						...
						<td>
                                <span
									className={`customer-status ${t.status?.toLowerCase() === "resolved" ? "completed" : "pending"}`}>
                                    {t.status}
                                </span>
						</td>
						<td>
							<button className="link" onClick={() => alert(t.complaintText)} style={{
								background: "none",
								border: "none",
								color: "#f97316",
								fontWeight: "600",
