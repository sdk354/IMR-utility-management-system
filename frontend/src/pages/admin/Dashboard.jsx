...
function Dashboard() {
	...
	const [stats, setStats] = useState({
		...
		todaysCollections: 0,
		revenueTrend: [] // Holds the [Month, Amount] arrays from Spring Boot
	});
	...
	// 4. Transform raw Java data [[Month, Amount], ...] into [{name, revenue}, ...]
	const chartData = stats.revenueTrend ? stats.revenueTrend.map(item => ({
		name: item[0],    // Month Name
		revenue: item[1]  // Revenue Amount
	})) : [];
			...
			<div className="admin-dashboard-grid">
				...
				<div className="admin-summary-card">
					...
					<div>
						...
						<div className="admin-summary-value">{loading ? "..." : formatCurrency(stats.outstandingBalance)}</div>
				...
				<div className="admin-summary-card">
					...
					<div>
						...
						<div className="admin-summary-value">{loading ? "..." : formatCurrency(stats.todaysCollections)}</div>
				...
				<div className="admin-card" style={{ flex: 1 }}>
					...
					<div className="admin-quick-actions">
						<Link to="/admin/customers/add">
							<button className="admin-quick-btn blue"><span className="icon-plus"></span> Add New Customer</button>
						</Link>
						<Link to="/admin/meters/register">
							<button className="admin-quick-btn indigo"><span className="icon-gauge"></span> Register New Meter</button>
						</Link>
						<Link to="/admin/payments/recordpayment">
							<button className="admin-quick-btn green"><span className="icon-credit-card"></span> Record Payment</button>
						</Link>
						<Link to="/admin/tariffs/add">
							<button className="admin-quick-btn purple"><span className="icon-tag"></span> Add Tariff</button>
						</Link>
						<Link to="/admin/complaints">
							<button className="admin-quick-btn orange"><span className="icon-message-alert"></span> View Complaints</button>
						</Link>
						<Link to="/admin/reports">
							<button className="admin-quick-btn gray"><span className="icon-chart"></span> View Reports</button>
						</Link>
					</div>
				</div>
			</div>
