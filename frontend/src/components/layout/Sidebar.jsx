...
function Sidebar({ isCustomer = false }) {
	...
	// Configuration for Admin Links
	const adminLinks = [
		{ path: '/admin/dashboard', label: 'Dashboard', icon: 'admin-icon-grid', roles: [ROLES.ADMIN, ROLES.FIELD, ROLES.CLERK, ROLES.MANAGER, ROLES.DEV] },
		...
		{ path: '/admin/complaints', label: 'Complaints', icon: 'admin-icon-message-alert', roles: [ROLES.ADMIN, ROLES.DEV] },
		{ path: '/admin/reports', label: 'Reports', icon: 'admin-icon-chart', roles: [ROLES.MANAGER, ROLES.DEV] },
	];

	return (
		<aside className={isCustomer ? 'customer-sidebar' : 'admin-sidebar'}>
				...
				{isCustomer ? (
					...
					<>
						{adminLinks.map((link) => {
							...
							return hasAccess ? (
								<NavLink key={link.path} to={link.path} className="admin-nav-link">
									<span className={link.icon}></span> {link.label}
								</NavLink>
							) : (
								<div key={link.path} className="admin-nav-link locked" title="Access Denied">
